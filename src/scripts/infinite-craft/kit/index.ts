...
function generateToolCombinations(...arrays: (string | null)[][]): string[] {
...
async function combine(
  ...
  if (logMessages) console.log("combine", first, "+", second);
  if (!first || !second) return;
  if (first.length > 30 || second.length > 30) return;
  [first, second] = [first.trim(), second.trim()].toSorted() as [
    string,
    string,
  ];
...
let __VUE__ = icWindow.document.querySelector(".infinite-craft").__vue__;
